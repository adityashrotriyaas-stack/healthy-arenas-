...
function Nav({ onAdminOpen, onDashboardOpen }) {
    ...
    const mobileItems = useMemo(() => [...links, ["Cart", "cart"], ["Order Now", "order"]], []);
        ...
        <nav style={{
            ...
            <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
                ...
                <button type="button" onClick={() => scroll("order")} className="nav-order" style={{
                    ...
                    onMouseEnter={e => { e.currentTarget.style.background = C.orangeDim; e.currentTarget.style.transform = "scale(1.03)"; }}
...
const SLOTS = ["10:00–10:30", "10:30–11:00", "11:00–11:30", "11:30–12:00", "12:00–12:30", "12:30–13:00", "13:00–13:30", "13:30–14:00", "14:00–14:30", "16:00–16:30", "16:30–17:00", "17:00–17:30", "17:30–18:00", "18:00–18:30", "18:30–19:00", "19:00–19:30", "19:30–20:00", "20:00–20:30", "20:30–21:00", "21:00–21:30"];
...
function OrderCTA() {
    ...
    const handleOrder = async () => {
        if (!address) return;
        if (!payment) {
            setLocMsg("Please select a payment method.");
            setLocType("error");
            return;
        }
        setChecking(true);
        ...
        const pos = await checkLocation();
        if (!pos) {
            setLocMsg("Allow location access to place an order.");
            setLocType("error");
            setChecking(false);
            return;
        }
        ...
        if (dist <= STORE.maxKm) {
            if (!slot) {
                setLocMsg("Please select a delivery time slot.");
                setLocType("error");
                setChecking(false);
                return;
            }
            const payLabel = payment === "online" ? "Online Payment" : "Cash on Delivery";
            if (payment === "online") {
                if (cartTotal <= 0) {
                    setLocMsg("Your cart is empty — add dishes from the menu to pay online.");
                    setLocType("error");
                    setChecking(false);
                    return;
                }
                setLocMsg("Opening secure payment...");
                try {
                    await openRazorpay({
                        amount: cartTotal,
                        onSuccess: () => {
                    ...
                    setLocMsg("Payment failed — try again.");
                    setLocType("error");
                    setChecking(false);
                    return;
                }
            ...
            setLocMsg(`Too far (${Math.round(dist)} km) — we deliver within ${STORE.maxKm} km of ${STORE.name}.`);
            setLocType("error");
        }
        setChecking(false);
    };
    ...
    return (
        <section id="order" ref={ref} style={{ background: C.bg, padding: "96px 5vw" }}>
            <div style={{
                ...
                <div style={{
                    ...
                    display: "flex", alignItems: "center", justifyContent: "center",
                    fontSize: 28,
                }}><Icon name="scooter" size={28} style={{ color: "#fff" }} /></div>
                <h2 style={{ fontFamily: "'Playfair Display',serif", fontWeight: 800, fontSize: "clamp(32px, 5vw, 56px)", color: C.cream, margin: "0 0 16px", letterSpacing: -1.5 }}>
                    Hungry right now?
                </h2>
                <p style={{ fontFamily: "'Inter',sans-serif", fontSize: 17, lineHeight: 1.7, color: C.creamDim, marginBottom: 36 }}>
                    Drop your address below and discover restaurants delivering to you in minutes.
                </p>
                ...
                {submitted ? (
                    <div>
                        <div style={{
                            ...
                            <div style={{ marginBottom: 12 }}><Icon name="party" size={40} style={{ color: C.green }} /></div>
                            <div style={{ fontFamily: "'Playfair Display',serif", fontWeight: 800, fontSize: 22, color: C.cream }}>Order placed!</div>
                            <div style={{ fontFamily: "'Inter',sans-serif", fontSize: 15, color: C.creamDim, marginTop: 8 }}>Delivering by {slot} · {payment === "online" ? <><Icon name="creditCard" size={14} style={{ verticalAlign: "middle", marginRight: 2 }} /> Paid online</> : <><Icon name="cash" size={14} style={{ verticalAlign: "middle", marginRight: 2 }} /> Cash on delivery</>}.</div>
                            <div style={{ fontFamily: "'Inter',sans-serif", fontSize: 13, color: C.creamDim, marginTop: 4 }}>Your food is on its way.</div>
                        </div>

                        {!feedbackSent ? (
                            <div style={{ background: C.bgCard, border: `1px solid ${C.border}`, borderRadius: 16, padding: "28px", textAlign: "center" }}>
                                <div style={{ fontFamily: "'Playfair Display',serif", fontWeight: 700, fontSize: 17, color: C.cream, marginBottom: 4 }}>
                                    How was your experience?
                                </div>
                                <div style={{ fontFamily: "'Inter',sans-serif", fontSize: 13, color: C.creamDim, marginBottom: 18 }}>
                                    ...
                                    {[1, 2, 3, 4, 5].map(n => (
                                        <button key={n} type="button" onClick={() => setFeedbackRating(n)}
                                            ...
                                            style={{
                                                ...
                                                transition: "transform 0.15s", transform: (feedbackHover || feedbackRating) >= n ? "scale(1.15)" : "scale(1)",
                                            }}
                                        ><Icon name="star" size={28} style={{ color: (feedbackHover || feedbackRating) >= n ? C.amber : "rgba(255,255,255,0.15)" }} /></button>
                                    ))}
                                </div>
                                <textarea value={feedbackText} onChange={e => setFeedbackText(e.target.value)}
                                    ...
                                    style={{
                                        width: "100%", background: C.bg, border: `1px solid ${C.border}`,
                                        borderRadius: 10, padding: "12px 14px", fontFamily: "'Inter',sans-serif",
                                        fontSize: 13, color: C.cream, outline: "none", resize: "vertical",
                                ...
                                <button type="button" onClick={() => {
                                    if (feedbackRating > 0) {
                                        try {
                                            ...
                                            existing.push({ rating: feedbackRating, text: feedbackText, date: new Date().toISOString() });
                                            localStorage.setItem("feedback", JSON.stringify(existing));
                                        } catch (e) {}
                                        setFeedbackSent(true);
                                    }
                                }} style={{
                                    background: feedbackRating > 0 ? C.orange : "rgba(255,255,255,0.1)",
                                    border: "none", cursor: feedbackRating > 0 ? "pointer" : "default",
                                    borderRadius: 50, padding: "10px 32px",
                                    fontFamily: "'Inter',sans-serif", fontSize: 14, fontWeight: 700,
                                    color: feedbackRating > 0 ? "#fff" : C.creamDim,
                                    transition: "all 0.2s",
                                }}>Submit Feedback</button>
                            </div>
                        ) : (
                            <div style={{ background: "rgba(245,166,35,0.08)", border: `1px solid rgba(245,166,35,0.25)`, borderRadius: 16, padding: "28px", textAlign: "center" }}>
                                <div style={{ fontFamily: "'Playfair Display',serif", fontWeight: 700, fontSize: 16, color: C.amber, marginBottom: 4 }}>
                    ...
                    <div>
                        ...
                        <div style={{ marginTop: 16, position: "relative", maxWidth: 520, marginLeft: "auto", marginRight: "auto" }}>
                            <button type="button" onClick={() => setShowSlots(o => !o)}
                                style={{
                                    width: "100%", background: C.bgCard, border: `1px solid ${C.border}`,
                                    borderRadius: 50, padding: "12px 18px",
                                    fontFamily: "'Inter',sans-serif", fontSize: 13, color: slot ? C.cream : "rgba(200,184,154,0.4)",
                                    textAlign: "left", cursor: "pointer",
                                    display: "flex", justifyContent: "space-between", alignItems: "center",
                                }}
                            >
                                {slot || "Select delivery time slot"}
                                <span style={{ color: C.creamDim, display: "flex", alignItems: "center" }}>{showSlots ? <Icon name="chevronUp" /> : <Icon name="chevronDown" />}</span>
                            </button>
                            {showSlots && (
                                <div style={{
                                    position: "absolute", top: "100%", left: 0, right: 0, zIndex: 10,
                                    background: C.bgCard, border: `1px solid ${C.border}`,
                                    borderRadius: 16, marginTop: 4, maxHeight: 200, overflow: "auto",
                                }}>
                                    {SLOTS.map(s => (
                                        <button key={s} type="button" onClick={() => { setSlot(s); setShowSlots(false); }}
                                            style={{
                                                width: "100%", background: slot === s ? "rgba(255,94,20,0.1)" : "none",
                        ...
                        <div style={{ marginTop: 16, maxWidth: 520, marginLeft: "auto", marginRight: "auto" }}>
                            <div style={{ display: "flex", gap: 10 }}>
                                {[
                                    { key: "online", label: "Pay Online", icon: "creditCard", sub: "UPI, Card, NetBanking" },
                                    { key: "cod", label: "Cash on Delivery", icon: "cash", sub: "Pay when it arrives" },
                                ].map(o => (
                                    <button key={o.key} type="button" onClick={() => setPayment(o.key)} style={{
                                        flex: 1, background: payment === o.key ? "rgba(255,94,20,0.1)" : C.bgCard,
                                        border: `1px solid ${payment === o.key ? C.borderO : C.border}`,
                                        borderRadius: 12, padding: "14px 12px", cursor: "pointer",
                                        textAlign: "center", transition: "all 0.2s",
                                    }}>
                                        <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 8, fontFamily: "'Playfair Display',serif", fontWeight: 700, fontSize: 14, color: payment === o.key ? C.orange : C.cream, marginBottom: 4 }}>
                                            <Icon name={o.icon} size={18} style={{ color: payment === o.key ? C.orange : C.cream }} /> {o.label}
                                        </div>
                                        <div style={{ fontFamily: "'Inter',sans-serif", fontSize: 11, color: C.creamDim }}>{o.sub}</div>
                                    </button>
                                ))}
                            </div>
                        </div>
                ...
                <div style={{ fontFamily: "'Inter',sans-serif", fontSize: 12, color: "rgba(200,184,154,0.5)", marginTop: 16 }}>
                    Free delivery on your first 3 orders · No minimum order
                </div>
            </div>
        </section>
    );
